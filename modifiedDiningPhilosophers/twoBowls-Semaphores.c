...
int main(int argc, char* argv[]){
    ...
    sem_init(&bowlCounter[0], 0, 1);
    sem_init(&bowlCounter[1], 0, 1); //bowl counters will also be binary
    
    printf("Dining philosophers in C with utlization of semaphores for shared resources and having 2 shared sauce bowls!\n");
...
void* philosopher(void* phNum) {
    ...
    while (1) {
        ...
        if (eat) {
            ...
            printf("P %d will try to eat from bowl 0\n", phId);

            if ((rc = sem_trywait(&bowlCounter[0])) != 0){
                perror("error, failed to pick bowl 0: ");
                printf("P %d will try picking bowl 1 now\n", phId);
                if ((rc = sem_trywait(&bowlCounter[1])) != 0){
                    perror("error, failed to pick bowl 1 too: ");
                    printf("P %d will not eat as he didn't get any bowl free\n", phId);
                    ...
                    printf("P %d is eating from bowl 1\n", phId);

                    sleep(rand() % 5 + 1);

                    printf("P %d has finished eating from bowl 1\n", phId);

                    if ((rc = sem_post(&bowlCounter[1])) != 0){
                        perror("error: "); exit(1);
                    }
                    if ((rc = sem_post(&Chopstick[rStick])) != 0){
                        perror("error: "); exit(1);
                ...
                printf("P %d is eating from bowl 0\n", phId);

                sleep(rand() % 5 + 1);

                printf("P %d has finished eating from bowl 0\n", phId);

                if ((rc = sem_post(&bowlCounter[0])) != 0){
                    perror("error: "); exit(1);
                }
                if ((rc = sem_post(&Chopstick[rStick])) != 0){
                    perror("error: "); exit(1);
